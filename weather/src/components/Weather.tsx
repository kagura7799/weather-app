...
import thunderstorm from '../../public/images/Thunderstorms.png';
...
const Weather = () => {
    ...
    const selectCity = () => {
        if (initCityName.length === 0 || initCityName.includes(' '))
    ...
    useEffect(() => {
        try {
            axios.get(WEATHER_API).then(res => {
                ...
                const temperatureWithSign =
                    parseFloat(celciusTemp) >= 0
                        ? `${celciusTemp}°`
                        : `${celciusTemp}°`;
                setData({
                    ...data,
                    celcius: temperatureWithSign,
    ...
    return (
        <div className='rounded-lg mt-24 bg-cover pb-5 pt-5 text-amber-50'>
            <section className=''>
                <div className='rounded-lg text-center bg-white/20 backdrop-blur-lg backdrop-opacity-50 mx-auto w-1/4'>
                    <div className='mx-auto'>
                        ...
                        >
                            <img
                                ...
                                alt='search'
                                width='40'
                                className='absolute top-14 right-14 w-6 h-6'
                            />
                        </button>
                        {loading ? (
                            <p>Загрузка...</p>
                                        ...
                                        src={
                                            data.weatherStatus === 'Clear'
                                                ...
                                                : data.weatherStatus ===
                                                  'Thunderstorm'
                                                ? thunderstorm
                                                : data.weatherStatus === 'Smoke'
                                                ? fog
                                                : ''
                                        }
                                        alt='weather-icon'
                                        width='230'
                                        className='mr-auto ml-auto mb-10 mt-10'
                                    />
                                )}
                                <h1 className='text-5xl'>{data.celcius}</h1>
                                <div className='flex text-center justify-center mt-5 pb-10'>
                                    {nameCity !== '' ? (
                                        <img src={maps} alt='map' width='25' />
                                    ) : (
                                        ''
                                    )}
                                    <h3 className='text-1xl my-auto'>
                                        {nameCity}
                                    </h3>
                                </div>
                            </div>
